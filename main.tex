...
\begin{equation}
    s(x,y) = \sum_{n1=-\infty}^{{\infty}}\sum_{n2=-\infty}^{{\infty}}\delta(x-nD_x,y-n_2D_y),
\end{equation}
...
Ok, you already know to get the fourier series coefficents you integrate the singla over one period and divide by the area of the period:
\begin{equation}
    a_{k_,k_2} = \frac{1}{D_xD_y}\int_{-Dy/2}^{Dy/2}\int_{-Dx/2}^{Dx/2}s(x,y)e^{-j(k_1\omega_1^0x + k_2\omega_2^0y)}dx\,dy
\end{equation}
...
\begin{align}
    a_{k_,k_2} &= \frac{1}{D_xD_y}\int_{-Dy/2}^{Dy/2}\int_{-Dx/2}^{Dx/2}\delta(x,y)e^{-j(k_1\omega_1^0x + k_2\omega_2^0y)}dx\,dy\notag\\
...
Transformscan be expressed as matrix operations on rows, then columns:
\begin{equation*}
    F = W^TIW
\end{equation*}
...
\begin{align*}
    ...
    \text{Nquist Limit: }& = \frac{\Omega_0}{N}\frac{1}{2} = \frac{\Omega}{2} = \frac{\pi}{2D} 
\end{align*}
...
where $w$ is the Hamming window. In Fig.~\ref{fig:hamming} we see the FIR filter with rectange and hamming window. Hamming sacrifices slower roll off to remove the ripple.
...
\end{equation}
where $\sigma$ affects the bandwidth. Need to specify the number of taps and normalise the filter coefficients so that the sum to 1. $h = h/\text{sum}(h)$. With this can get wat with shorter filter (15 taps). 
